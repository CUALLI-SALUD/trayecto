...
export default function Trayecto() {
  const [estudios, setEstudios] = useState<Estudio[]>([
    ...
    { id: 4, nombre: "Rayos X", tiempo: 30, estado: "pendiente", desc: "Se requiere vestimenta cómoda." },
    { id: 5, nombre: "Electrocardiograma", tiempo: 15, estado: "pendiente", desc: "Último paso de tu trayecto hoy." },
  ]);
  ...
  // LÓGICA DE NOTIFICACIONES (Pop-up Verde cada 15 segundos)
  useEffect(() => {
    const interval = setInterval(() => {
      setPopup({
        titulo: "¡Ruta Optimizada!",
        mensaje: "Una sala se ha liberado antes de tiempo. Hemos movido tus estudios para reducir tu espera total."
      });

      setEstudios(prev => {
        const completados = prev.filter(e => e.estado === "completado");
        const pendientes = prev.filter(e => e.estado !== "completado");
        
        if (pendientes.length < 2) return prev; // Si ya no hay mucho que optimizar, no hace nada
        
        // Magia: Toma el último estudio, lo pone al principio de los pendientes y le baja el tiempo
        const ultimo = pendientes[pendientes.length - 1];
        const resto = pendientes.slice(0, -1);
        
        return [
          ...completados, 
          { ...ultimo, tiempo: Math.max(5, ultimo.tiempo - 5) }, 
          ...resto
        ];
      });
      ...
      <div className="relative pl-4 mt-2 pb-6">
        ...
        {estudios.map((e) => (
          <div key={e.id} className="mb-5 relative flex items-start gap-4 transition-all duration-700 transform">
            ...
            {/* ICONO DEL TIMELINE */}
            <div className={`w-9 h-9 rounded-full flex items-center justify-center z-10 shrink-0 mt-2 shadow-sm transition-colors duration-500 ${
            ...
            {/* TARJETA DEL ESTUDIO */}
            <div className={`flex-1 bg-white p-4 rounded-2xl shadow-sm border transition-all duration-500 ${
